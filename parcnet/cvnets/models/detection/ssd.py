...
class SingleShotDetector(BaseDetection):
    ...
    def __init__(self, opts, encoder: BaseEncoder):
        ...
        last_classes = getattr(opts, "model.detection.last_classes", None)
        print("last_classes", last_classes)
        if (last_classes):
            self.ssd_heads += [torch.nn.Linear(self.ssd_heads[-1].in_channel, last_classes)]
    ...
    def add_arguments(cls, parser: argparse.ArgumentParser):
        ...
        group.add_argument('--model.detection.last_classes', type=int, default=None,)
